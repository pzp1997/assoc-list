module Test.UniqueList exposing (tests)

import Basics exposing (..)
import Expect
import List exposing ((::))
import Test exposing (..)
import UniqueList as Set exposing (Set)
...
insertTests =
    ...
    , test "leaves set of 100 intact if it contains given element" <|
        \() ->
            Expect.equal
                (Set.fromList
                    (42 :: List.range 1 41 ++ List.range 43 100)
                )
                (Set.insert 42 set1To100)
    ]
...
foldlTests =
    [ test "with insert and empty set acts as identity function" <|
        \() ->
            Expect.equal (Set.fromList (List.reverse (List.range 1 100)))
                (Set.foldl Set.insert Set.empty set1To100)
...
foldrTests =
    ...
    , test "folds set elements in insertion order" <|
